...
%  USAGE
%
%    smoothed = general.smooth(data, smooth)
%
%    data           data to smooth
...
function smoothed = smoothData(data, smooth)

	%  vector = min(size(data)) == 1;
	vector = isvector(data);
	matrix = (~vector & length(size(data)) == 2);
	if ~vector && ~matrix,
		error('Smoothing applies only to vectors or matrices (type ''help <a href="matlab:help general.smooth">general.smooth</a>'' for details).');
	end
	...
	if nargin < 2,
		error('Incorrect number of parameters (type ''help <a href="matlab:help general.smooth">general.smooth</a>'' for details).');
	end

% 	if ~isdvector(smooth, '>0') || (matrix && length(smooth) > 2) || (vector && length(smooth) ~= 1),
% 		error('Incorrect value for property ''smooth'' (type ''help <a href="matlab:help general.smooth">general.smooth</a>'' for details).');
% 	end
	...
	% 1) Vertical kernel
    % vKernel = fspecial('gaussian', [1 length(-vKernelSize:vKernelSize)], smooth(1));
