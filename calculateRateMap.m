...
function map = calculateRateMap(v, z, varargin)
    ...
    checkSmooth = @(x) isdvector(x, '>=0') && length(x) <= 2;
    checkScalar = @(x) isdscalar(x, '>=0');
    checkType = @(x) isstring(x, 'circular', 'linear');
    checkVector = @(x) isivector(x, '>0');
    ...
    addRequired(inp, 'v');
    addRequired(inp, 'z');
    addParameter(inp, 'smooth', defaultSmooth);
    addParameter(inp, 'binWidth', defaultBinWidth);
    addParameter(inp, 'minTime', defaultMinTime, checkScalar);
    addParameter(inp, 'maxGap', defaultMaxGap, checkScalar);
    addParameter(inp, 'type', defaultType);
    ...
    for p = 1:numPosGroups
        ...
        if isempty(y)
            ...
            time = accumulate([x y], dt, [nBinsX nBinsY])'; % how much time animal spent at a specific bin
            time = round(time * 10e6) / 10e6; % remove floating numbers artefacts
        end
        ...
        for s = 1:numIter
            ...
            if isempty(y)
                ...
                if isempty(z)
                    ...
                    Nspikes = accumulate([x y], n, [nBinsX nBinsY]);
                end
                map(m).countRaw = Nspikes'; % how many times (# spikes) animal was at a specific bin
                map(m).timeRaw = time;
                ...
                map(m).z = smoothData(zRaw, smooth);
                map(m).time = smoothData(map(m).timeRaw, smooth);
                map(m).count = smoothData(map(m).countRaw, smooth);
            end
