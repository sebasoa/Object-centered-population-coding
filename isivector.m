...
%  USAGE
%
%    test = helpers.isivector(x,test1,test2,...)
%
%    x              parameter to test
...
%  SEE ALSO
%
%    See also heleprs.isdmatrix, helpers.isdvector, helpers.isdscalar,
%    helpers.isimatrix, heleprs.isiscalar, helpers.isstring, helpers.islscalar,
%    helpers.islvector, helpers.islmatrix.
%
...
if nargin < 1,
  error('Incorrect number of parameters (type ''help <a href="matlab:help helpers.isivector">helpers.isivector</a>'' for details).');
end
...
for i = 1:length(varargin),
	try
		...
		elseif helpers.isstring(varargin{i}, '>', '>=', '<', '<='),
			dx = diff(x); %#ok<NASGU>
		...
		error(['Incorrect test ''' varargin{i} ''' (type ''help <a href="matlab:help helpers.isivector">helpers.isivector</a>'' for details).']);
	end
end
